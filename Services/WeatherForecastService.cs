...
public class WeatherForecastService
{
    public async Task<List<WeatherForecast>> GetForecastAsync(DateOnly startDate)
    {
        ...
        var response = await cl.GetFromJsonAsync<Root>("/v1/forecast.json?key=ead1ea46da534be3b64141536242704&q=Wroclaw&days=7");
        ...
        foreach (var day in response.forecast.forecastday)
        {
            output.Add(new WeatherForecast()
            {
                ...
                TemperatureC = Convert.ToInt32(Math.Round(day.day.avgtemp_c, 0)),
